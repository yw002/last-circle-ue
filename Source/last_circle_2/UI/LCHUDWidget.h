...
public:
    virtual void NativeConstruct() override
    {
        ...
        // Load engine default font — without this, UTextBlock will not render
        DefaultFontObj = LoadObject<UObject>(nullptr, TEXT("/Engine/EngineFonts/Roboto.Roboto"));
        ...
        WeaponNameText = MakeText(TEXT("WeaponName"), FLinearColor(0.7f, 0.7f, 0.7f), 20);
        if (WeaponNameText)
        {
        ...
        // Ammo text (bottom-right) — large white
        AmmoText = MakeText(TEXT("Ammo"), FLinearColor::White, 36);
        if (AmmoText)
        {
        ...
        ReloadText = MakeText(TEXT("Reloading"), FLinearColor(1.f, 0.8f, 0.f), 24);
        if (ReloadText)
        {
        ...
        static const TCHAR* Chars[4] = { TEXT("\xE2\x94\x82"), TEXT("\xE2\x94\x82"), TEXT("\xE2\x94\x80\xE2\x94\x80"), TEXT("\xE2\x94\x80\xE2\x94\x80") };
        for (int32 i = 0; i < 4; ++i)
        {
            CrosshairTexts[i] = MakeText(FName(*FString::Printf(TEXT("XHair%d"), i)), FLinearColor(0.1f, 1.f, 0.1f, 0.9f), 18);
            if (CrosshairTexts[i])
            {
                CrosshairTexts[i]->SetText(FText::FromString(Chars[i]));
        ...
        AliveText = MakeText(TEXT("Alive"), FLinearColor::White, 16);
        if (AliveText)
        {
    ...
    void UpdateCrosshair()
    {
        ...
        SetPos(0, 0.f, 0.f - Gap - LineLen, 3.f, 2.f);
        SetPos(1, 0.f, 0.f + Gap, 3.f, 2.f);
        SetPos(2, 0.f - Gap - LineLen, 0.f, 4.f, 2.f);
        SetPos(3, 0.f + Gap, 0.f, 4.f, 2.f);
    }
...
private:
    UTextBlock* MakeText(FName Name, const FLinearColor& Color, int32 Size)
    {
        UTextBlock* TB = WidgetTree->ConstructWidget<UTextBlock>(UTextBlock::StaticClass(), Name);
        if (TB)
        {
            TB->SetColorAndOpacity(FSlateColor(Color));
            if (DefaultFontObj)
            {
                FSlateFontInfo Font(DefaultFontObj, Size);
                TB->SetFont(Font);
            }
        }
        return TB;
    ...
    UPROPERTY() UObject* DefaultFontObj = nullptr;
